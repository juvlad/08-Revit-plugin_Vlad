...
«Проект» — пять кнопок, работают **только в проекте** (`.rvt`). Тестового проекта нет — проверка ручная, в Revit.
...
- **Рабочий набор для перехода команда создаёт, если его нет.** В новом разделе
  «00_Shared levels and grids» часто ещё не заведён, а перейти в несуществующий набор нельзя;
  молчаливый пропуск означал бы, что кнопка не сделала главного. Имя перед созданием сверяется
  через `WorksetTable.IsWorksetNameUnique` — занятое набором другого вида уходит в отчёт.
...
`SITE` (имя площадки), `WORKSET` (набор, в который переходить) и по галочке на шаг — `ACQUIRE`,
`RENAME`, `PIN`, `ACTIVATE`, `MONITOR`. Перезаписывается при любом закрытии окна. Наборов, как
у `links\`, здесь нет и не нужно: базовый файл один. Имена серверов Revit Server окно берёт и
пополняет в `links\_settings.txt` — набирать их дважды пользователь не должен.

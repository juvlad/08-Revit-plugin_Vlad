...
«Проект» — четыре кнопки, работают **только в проекте** (`.rvt`). Тестового проекта нет — проверка ручная, в Revit.
...
(можно подобрать по коду раздела в имени модели); новые связи создаются одной транзакцией и закрепляются булавкой, существующие перезагружаются `LoadFrom` вне транзакции |
| Проект | «Авто размеры» | `AutoDimensionCommand` | по каталогу видов ниток (габарит / проёмы / оси проёмов / перегородки / грани стен / всё вместе) расставляет размеры по сторонам выбранных помещений; набор ниток собирается кнопкой «Взять образец…» (разбор уже расставленных вручную размеров) или руками, сохраняется шаблоном |
...
- **Набор проекта для связи можно подобрать по имени модели** (`DisciplineCatalog`, галочка
  ...
  отдельным словом (`01_Link_OV`, `01_Связи_OV`, но не `Provod`). Ровно одно совпадение → набор
  ставится, несколько → строка помечается в «Состоянии» и выбор за пользователем, ноль → молчание.
...
- **Углы нитки берутся не с торца своей стены, а с продольной грани соседней.** У стены в реальном
  ...
  плоская на всю свою длину, поэтому `DimensionReferenceCollector.Corners` берёт угол оттуда: у
  соседней по петле стороны (`LoopNeighbor`) ищется её ближайшая к общему углу грань. Соседа нет,
  он не перпендикулярен или у него тоже нет подходящей грани — тогда запасной вариант: крайняя точка
  среди собственных граней стороны, а не отказ строить нитку целиком.
- **Все точки внутри нитки, кроме двух угловых, отсекаются по диапазону между углами
  (`DimensionReferenceCollector.Interior`), а не только дедупом по 1 мм.** Стена, не идеально
  подрезанная в углу (нет чистого митра — торец остаётся плоским и параллельным оси), даёт свою
  угловую грань ещё раз через `OwnWallHits`, вдобавок к настоящему углу от соседней стены
  (см. предыдущий пункт); эта вторая точка обычно смещена от истинного угла больше чем на 1 мм
  (на глубину соседней стены) — обычный дедуп её не схлопывает, и рядом с углом появляется лишняя
  короткая засечка размером примерно в толщину стены. `Interior` отбрасывает у любого «лишнего»
  набора точек (собственные грани стены для «Проёмы», стыки для «Грани стен», оси для «Оси проёмов»,
  грани перегородок для «Перегородки»/«Всё вместе») всё, что не строго между двумя угловыми точками
  нитки — сами углы при этом всегда берутся из `Corners`, а не из этого набора.
...
пишется в файл целиком. Имя набора «_settings» занято этим файлом:
...
шаблона), `BOUNDARY`, `OUTWARD`, `REMOVE_PREVIOUS`. Имя «_settings» занято этим файлом по тому же
правилу, что у `links\`.
